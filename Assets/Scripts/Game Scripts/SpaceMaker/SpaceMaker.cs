...
public class SpaceMaker : MonoBehaviour
{
	...
	void Start ()
	{
		...
		for (int n = 0; n <numberOfPlanes; n++) {
			...
			makeSpacePart (n, rx, ry, rz, distance, correctedScale, scale, colorMix, textureNum, light, spaceTexBrightnessModifer);
	...
	private void makeSpacePart (int n, int rx, int ry, int rz, float distance, float correctedScale, float scale, Color colorMix, int t, bool light, float brightness)
	{
		
		GameObject spacePart = Instantiate (plane, new Vector3 (0, 0, 0), Quaternion.identity) as GameObject;		
		
		spacePart.layer=9;
		
		spacePart.GetComponent<SpaceCtrl> ().setName (n);
		spacePart.GetComponentInChildren<SpacePlane> ().setDistance (distance);
		spacePart.GetComponentInChildren<SpacePlane> ().setColor (colorMix);
		spacePart.GetComponentInChildren<SpacePlane> ().setTexture (spaceParts [t].spaceTex);
		
		spacePart.transform.localEulerAngles = new Vector3 (rx, ry, rz);
		spacePart.transform.localScale = new Vector3 (scale, scale, 1);
		spacePart.transform.parent = transform;
		...
		if (light) {
			...
			lightGameObject.transform.parent = spacePart.transform;
